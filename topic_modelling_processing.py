...
def main():
	...
	for fileids in sql_result:
		...
		fetch_terms = mycursor.execute(" SELECT word FROM body_term_frequency WHERE file_id = %s ", fileids)
		...
		fetch_terms_tfidf = mycursor.execute("SELECT a.file_id, b.word, b.tf_idf FROM body_tfidf_table AS b \
										INNER JOIN body_term_frequency AS a ON b.word = a.word \
										WHERE a.file_id = %s ORDER BY b.tf_idf DESC LIMIT 0, " + str(tfidf_limit), fileids)
		sql_res = mycursor.fetchall()

		word_list_tfidf = []
		for w in sql_res:
			file_id, word, tf_idf = w[0], w[1], w[2]
			#print("file id: {}\t word: {} \t tf_idf: {}".format(file_id, word, tf_idf))
			word_list_tfidf.append(word)

		file_list.append(file_id)

		word_bucket_tfidf.append(word_list_tfidf)	
	#print(word_bucket)
	#print(file_list)

	#create a dictionary
	wordIDs = corpora.Dictionary(word_bucket)
	wordIDs_tfidf = corpora.Dictionary(word_bucket_tfidf)
	...
	corpus_tfidf = [wordIDs_tfidf.doc2bow(word) for word in word_bucket_tfidf]

	#[[(0, 1), (1, 1), (2, 2)...]] -- wordID, frequency  tuple
	#print(corpus)
	#see human_readble dictionary - word, frequency
	#print([[(wordIDs[id], freq) for id, freq in corp] for corp in corpus])
	#[[('abroad', 1), ('access', 1), ('accessible', 1), ('address', 1), ('answer', 1),...]]
	...
	#plt.show()	
	plt.savefig(r'../enron analysis/statistics/topic model stats/chart_plot_'+str(tfidf_limit)+'_'+str(limit)+'.png')
	...
	for i, row in enumerate(optimal_model[corpus]):
		...
		for topic_prob in row:
			topics = topic_prob[0]
			probs = topic_prob[1]
			insert_topic_prop = mycursor.execute(" INSERT INTO topic_prob_table VALUES (%s, %s, %s) ", (i, topics, round(probs, 4)))
			mydb.commit()
		...
		for j, (topic_num, topic_prob) in enumerate(row):
			...
			#print('\n', topic_num)
			#print(word_perc)
			check_topic_existence = mycursor.execute("SELECT topic_number FROM topic_term_prob_table WHERE topic_number = '%s' " % topic_num)
			f = mycursor.fetchall()
			if not f:
				for terms in word_prob:
					word = terms[0]
					prob = terms[1]
					insert_into_table = mycursor.execute("INSERT INTO topic_term_prob_table VALUES (%s, %s, %s)", (str(topic_num), str(word), str(round(prob, 4))))
					mydb.commit()

			if j == 0:  #=> dominant topic

				#sent_topics_df = sent_topics_df.append(pd.Series([int(topic_num), round(prop_topic, 4), topic_keywords]), ignore_index=True)
				insert_into_table = mycursor.execute("INSERT INTO topic_model_table VALUES (%s, %s, %s)", (i, str(topic_num), str(round(topic_prob, 4))))
				mydb.commit()					
	...
	pyLDAvis.save_html(visualisation, '../enron analysis/statistics/topic model stats/pyldavis_'+str(tfidf_limit)+'_'+str(limit)+'.html')
...
It is note worthy that, unlike LDA which provides probabilistic values for each topic, NMF have a different approach. NMF provide
probabilistic value for only the topics it deems fit enough. For instance, for a-10 topic model, NMF may issue
just 5 probabilistic values for only the topics that best represent the document; other will be ignored. 
In our implemetation, we programmatically handled the topics that were not represented in a document by assigning 
probability value 0.
