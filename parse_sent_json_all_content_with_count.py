...
for src_ in source_list:
	...
	json_file = "enron analysis/sent_json_all_content_with_count/"+from_name+"-sent.json"
		...
		for to1 in email_list_set:
			...
			for filename in os.listdir(src_):
				...
				with open(src_file, 'r') as f:        			    
					...
					email_body = mail_cont.get_payload()
					...
					if to_email2 == to1:
						...
						if email_body:
							email_body = body_preprocess(email_body)
						else:
							email_body = 'Null'

						count_sub = word_count(subjects)
						count_body = word_count(email_body)
						...
						json_content2 = '{"email": "'+str(to_email2)+'", "name": "'+str(to_name)+'", "label": "'+str(concat_f_char)+'", "subject": { "sub": "'+str(subjects)+'", "date": "'+str(date)+'", "sub_count": "'+str(count_sub)+'" }, "body": { "main_body": "'+str(email_body)+'", "body_count": "'+str(count_body)+'", "filename": "'+src_base+'" }}, '

			#make sure the currently looped dict is not empty before appending			
			if json_content2 != '':	
				#open the file and append the dics as json																
				with open("enron analysis/sent_json_all_content_with_count/"+from_name+"-sent.json", "a") as f:
					#dump dict as json			
	...
	with open("enron analysis/sent_json_all_content_with_count/"+from_name+"-sent.json", "r+") as f:
