...
def main():
	...
	def compute_coherence_value(dictionary, corpus, texts, limit, start, step):

		coherence_values = []
		model_list = []
		for num_topics in range(start, limit, step):
			model = Nmf(corpus=corpus, num_topics=num_topics, id2word=dictionary, chunksize=2000, passes=5, kappa=.1, minimum_probability=0.01, w_max_iter=200, w_stop_condition=0.0001, h_max_iter=50, h_stop_condition=0.001, eval_every=10, normalize=True, random_state=42)
	...
	model_list, coherence_values = compute_coherence_value(wordIDs, corpus, word_bucket, limit, start, step)
	...
	#plt.show()	
	plt.savefig(r'../enron analysis/statistics/topic model stats/nmf_chart_plot_comm_graph_'+str(limit)+'.png')
	...
	optimal_model.save('nmf_main.model')
	...
	for i, row in enumerate(optimal_model[corpus]):
		...
		for topic_prob in row:
			topics = topic_prob[0]
			probs = topic_prob[1]
			insert_topic_prop = mycursor.execute(" INSERT INTO comm_graph_nmf_topic_prob_table VALUES (%s, %s, %s) ", (i, topics, round(float(probs), 4)))
			mydb.commit()
		...
		for j, (topic_num, topic_prob) in enumerate(row):
			...
			#print('\n', topic_num)
			#print(word_perc)
			check_topic_existence = mycursor.execute("SELECT topic_number FROM comm_graph_nmf_topic_term_prob_table WHERE topic_number = '%s' " % topic_num)
			f = mycursor.fetchall()
			if not f:
				for terms in word_prob:
					word = terms[0]
					prob = terms[1]
					insert_into_table = mycursor.execute("INSERT INTO comm_graph_nmf_topic_term_prob_table VALUES (%s, %s, %s)", (str(topic_num), str(word), str(round(float(prob), 4))))
					mydb.commit()

			if j == 0:  #=> dominant topic

				#sent_topics_df = sent_topics_df.append(pd.Series([int(topic_num), round(prop_topic, 4), topic_keywords]), ignore_index=True)
				insert_into_table = mycursor.execute("INSERT INTO comm_graph_nmf_topic_model_table VALUES (%s, %s, %s)", (i, str(topic_num), str(round(topic_prob, 4))))
				mydb.commit()					
