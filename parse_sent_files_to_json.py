...
for src_ in source_list:
	...
	if not isFIleExist:
		...
		json_content = '{"from": {"email": ' + '"'+from_email+'", "name": "'+from_name+'"}, "to": ['
			...
			for filename in os.listdir(src_):
				...
				with open(src_file, 'r') as f:        			    
					...
					if to_email2 == to1:
						...
						json_content2 = '{"email": "'+str(to_email2)+'", "name": "'+str(to_name)+'", "subject": [' + ', '.join([ '"'+ str(a) + '"' for a in unique_subject_list ]) +']}, '
	...
	with open("enron analysis/sent/"+from_name+"-sent.json", "r+") as f:
